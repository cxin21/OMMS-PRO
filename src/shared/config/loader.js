...
export class ConfigLoader {
    ...
    loadDefaults() {
        ...
        if (!existsSync(defaultConfigPath)) {
            throw new Error(`config.default.json not found at ${defaultConfigPath}. ` +
    ...
    mergeConfigs(...configs) {
        const validConfigs = configs.filter(c => c && typeof c === 'object');
        if (validConfigs.length === 0) {
            return this.loadDefaults();
        }
        // 从默认配置开始合并
        const baseConfig = this.loadDefaults();
        const merged = ObjectUtils.merge(baseConfig, ...validConfigs);
        return merged;
    }
    /**
     * 保存配置到文件
     ...
     * @param options - 保存选项
     */
    async saveToFile(config, filePath, options) {
        const path = filePath ? resolvePath(filePath) : this.getDefaultConfigPath();
