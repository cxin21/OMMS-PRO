...
export class ConfigManager {
    ...
    getConfig(path) {
        ...
        return PathUtils.getByPath(this.config, path);
    }
    /**
     * 获取配置（必须存在）
     ...
     * @returns 配置值
     */
    getConfigOrThrow(path) {
        if (!this.initialized || !this.config) {
            throw new Error(`ConfigManager not initialized. Call initialize() before accessing '${path}'. ` +
                `Services must not be constructed before config init.`);
        }
        const value = PathUtils.getByPath(this.config, path);
        if (value === undefined || value === null) {
            throw new Error(`Config path '${path}' not found in configuration. Ensure config.default.json contains this key.`);
        }
        return value;
    }
    /**
     ...
     */
    async updateConfig(path, value, persist = true) {
        if (!this.initialized) {
            throw new Error('ConfigManager not initialized');
        }
        if (!this.config) {
        ...
        if (!validation.valid) {
            ...
            this.logger.error('Configuration update validation failed', { path, value, errors: validation.errors });
            throw error;
        }
        // 应用更新
        PathUtils.setByPath(this.config, path, value);
        // 记录变更
        const changeEvent = {
            path,
            oldValue,
        ...
        this.logger.info('Configuration updated', { path, value, persisted: persist });
    }
    /**
     * 重置配置
     *
    ...
    async resetConfig(path) {
        ...
        if (!path) {
            ...
            PathUtils.setByPath(this.config, path, newValue);
            const changeEvent = {
                path,
                oldValue,
                newValue,
