...
export class FileUtils {
   ...
   * 生成临时文件路径
   */
  static tempPath(prefix = 'tmp', extension = ''): string {
   ...
   * 获取文件最后修改时间
   */
  static async getModifiedTime(filePath: string): Promise<number> {
    const stats = await stat(filePath);
   ...
   * 获取文件创建时间
   */
  static async getCreatedTime(filePath: string): Promise<number> {
    const stats = await stat(filePath);
   ...
   * 获取文件访问时间
   */
  static async getAccessTime(filePath: string): Promise<number> {
    const stats = await stat(filePath);
   ...
   * 判断文件是否过期
   */
  static async isFileExpired(filePath: string, maxAge: number): Promise<boolean> {
   ...
   * 从部分构建路径
   */
  static fromPathParts(parts: string[], absolute = false): string {
