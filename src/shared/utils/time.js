...
export class TimeUtils {
     ...
     * @returns 相对时间字符串
     */
    static relative(timestamp, now = Date.now()) {
     ...
     * @returns 是否过期
     */
    static isExpired(timestamp, ttl, now = Date.now()) {
     ...
     */
    static getRemainingTime(timestamp, ttl, now = Date.now()) {
     ...
     * @returns 剩余时间字符串
     */
    static formatRemaining(timestamp, ttl, now = Date.now()) {
     ...
     * 添加年数
     */
    static addYears(timestamp, years) {
        const date = new Date(timestamp);
     ...
     * 格式化 Unix 时间戳
     */
    static formatUnix(timestamp, format) {
     ...
     * @returns 毫秒差
     */
    static since(timestamp, now = Date.now()) {
     ...
     * @returns 天数差
     */
    static daysSince(timestamp, now = Date.now()) {
     ...
     * @returns 小时差
     */
    static hoursSince(timestamp, now = Date.now()) {
     ...
     * @returns 是否在范围内
     */
    static isInRange(timestamp, start, end) {
     ...
     * 获取星期几的中文名称
     */
    static getDayOfWeekName(timestamp) {
     ...
     * 获取月份的天数
     */
    static getDaysInMonth(timestamp) {
        const date = new Date(timestamp);
     ...
     * 格式化时间范围
     */
    static formatRange(start, end, format) {
     ...
     * 判断是否是未来时间
     */
    static isFuture(timestamp, now = Date.now()) {
        return timestamp > now;
    }
    /**
     * 判断是否是过去时间
     */
    static isPast(timestamp, now = Date.now()) {
        return timestamp < now;
    }
}
