 ...
 * @returns 方法装饰器
 */
export function withErrorBoundary(serviceName: string): MethodDecorator {
  return function (
    target: any,
    ...
    descriptor.value = async function (this: { logger: ILogger }, ...args: any[]) {
      ...
      try {
        ...
        logger.error(
          `[${serviceName}] ${String(propertyKey)} failed`,
          err,
          {
            correlationId,
            operation: String(propertyKey),
 ...
 * ```
 */
export function wrapWithErrorBoundary<T extends (...args: any[]) => Promise<any>>(
  logger: ILogger,
  operation: string,
  fn: T,
): T {
