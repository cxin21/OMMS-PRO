 ...
 * 每个服务的专用日志文件路径
 */
const DEFAULT_SERVICE_LOG_CONFIGS: Record<string, { filePath: string }> = {
  // Memory Services
 ...
 * 服务日志配置映射（运行时可配置）
 */
const SERVICE_LOG_CONFIGS: Record<string, { filePath: string }> = { ...DEFAULT_SERVICE_LOG_CONFIGS };

/**
 * 从配置初始化服务日志配置
 * 允许通过配置文件覆盖默认的日志路径
 */
function initializeServiceLogConfigs(): void {
  try {
    const serviceLogsConfig = config.getConfig<Record<string, string>>('logging.services');
 ...
 */
const BASE_SERVICE_CONFIG: Partial<LoggingConfig> = {
  output: 'both',
  enableConsole: true,
  enableFile: true,
  enableRotation: true,
 ...
 */
function getLogLevel(): 'trace' | 'debug' | 'info' | 'warn' | 'error' {
  try {
    if (config.isInitialized()) {
      const loggingConfig = config.getConfig<{ level?: 'trace' | 'debug' | 'info' | 'warn' | 'error' }>('logging');
 ...
 */
export function registerServiceLogConfig(serviceName: string, filePath: string): void {
  SERVICE_LOG_CONFIGS[serviceName] = { filePath };
}
 ...
 * @returns 是否成功刷新
 */
export function refreshServiceLogConfigs(): boolean {
  try {
    if (!config.isInitialized()) {
      return false;
    }
 ...
 * @returns 当前日志级别
 */
export function getCurrentLogLevel(): 'trace' | 'debug' | 'info' | 'warn' | 'error' {
  return getLogLevel();
}
