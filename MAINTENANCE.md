...
1. ~~**extractionTimeout配置路径错误**~~ ✓ 已修复
   - 改为 `config.getConfig('memoryService.capture')`

2. ~~**HybridSearch权重硬编码**~~ ✓ 已修复
   - 改为 vectorWeight=0.7, bm25Weight=0.3

3. ~~**interaction-recorder.ts中await非Promise**~~ ✓ 已修复
   - 移除不必要的 await

4. ~~**scopeDegradation和reinforcement配置重复**~~ ✓ 已修复
   - 删除重复配置项

5. ~~**computeCosineSimilarity死代码**~~ ✓ 已修复
   - 已删除未使用的方法
...
1. **Graph层被排除在事务封装之外** (`memory-store-manager.ts`)
   - `TransactionManager` 未包含 Graph 层的写入操作
   - 可能导致多存储操作时数据不一致

2. **各存储层UPSERT语义不一致**
   - SQLiteMetaStore: DELETE then INSERT
   - VectorStore: DELETE then ADD
   - PalaceStore: 直接覆盖
   - GraphStore: SELECT then UPDATE/INSERT

### 中等问题

1. **LanceDB批量查询失败回退逻辑bug** (`vector-store.ts`)
   - `getByIds()` 批量查询失败后回退到串行
   - 但未重置 results 数组，可能残留部分结果

2. **PalaceStore.move() TOCTOU竞争** (`palace-store.ts`)
   - 检查源文件存在和执行 rename 之间存在时间窗口
   - 并发调用可能导致数据丢失

3. **GraphStore upsertNode() TOCTOU竞争** (`graph-store.ts`)
   - SELECT 和 UPDATE/INSERT 之间存在竞争条件
...
2. **JSON数组LIKE查询性能瓶颈** (`graph-store.ts`)
   - `removeMemory()` 和 `findRelated()` 使用 `LIKE '%memoryId%'`
   - 无法利用索引，数据量增长时性能下降

3. **本地缓存无分布式同步** (`cache-manager.ts`)
   - 纯内存 LRU 缓存，多实例部署时各实例独立
   - 可能导致跨实例数据不一致
